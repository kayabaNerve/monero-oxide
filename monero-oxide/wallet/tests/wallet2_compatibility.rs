...
async fn initialize_rpcs() -> (SimpleRequestRpc, SimpleRequestRpc, MoneroAddress) {
  let wallet_rpc = SimpleRequestRpc::new("http://127.0.0.1:18082".to_owned()).await.unwrap();
...
test!(
  ...
  (
    ...
    async |_, _, tx: Transaction, _, data: SimpleRequestRpc| {
      // confirm receipt
...
test!(
  ...
  (
    ...
    async |_, _, tx: Transaction, _, data: (SimpleRequestRpc, u32)| {
      // confirm receipt
...
test!(
  ...
  (
    ...
    async |_, _, tx: Transaction, _, data: (SimpleRequestRpc, SimpleRequestRpc, u32)| {
      // confirm receipt
      let _: EmptyResponse = data.0.json_rpc_call("refresh", None).await.unwrap();
      let transfer: TransfersResponse = data
        .0
...
test!(
  ...
  (
    ...
    async |_, _, tx: Transaction, _, data: (SimpleRequestRpc, [u8; 8])| {
      // confirm receipt
      let _: EmptyResponse = data.0.json_rpc_call("refresh", None).await.unwrap();
      let transfer: TransfersResponse = data
        .0
