...
test!(
  ...
  (
    ...
    async |_rpc: SimpleRequestRpc, block, tx: Transaction, mut scanner: Scanner, ()| {
...
test!(
  ...
  (
    async |rct_type, rpc: SimpleRequestRpc, _, _, outputs: Vec<WalletOutput>| {
    ...
    async |_rpc: SRR, block: SB, tx: Transaction, _, views: (ViewPair, ViewPair)| {
...
test!(
  ...
  (
    ...
    async |_rpc: SimpleRequestRpc,
           block,
           tx: Transaction,
           _,
           mut state: (Scanner, Vec<SubaddressIndex>)| {
...
test!(
  ...
  (
    async |rct_type, rpc: SimpleRequestRpc, _, addr, outputs: Vec<WalletOutput>| {
      use monero_wallet::rpc::FeePriority;
      ...
      add_inputs(rct_type, &rpc, vec![outputs.first().unwrap().clone()], &mut builder).await;
      builder.add_payment(addr, 1_0000);
    ...
    async |_rpc: SimpleRequestRpc, block, tx: Transaction, mut scanner: Scanner, ()| {
      ...
      outputs.sort_by(|x, y| x.commitment().amount.cmp(&y.commitment().amount));
      assert_eq!(outputs[0].commitment().amount, 1_0000);
      ...
      assert_eq!(proofs.base.fee, 1_000_000_000_000 - 1_0000 - 50000);
    },
  ),
);
...
test!(
  ...
  (
    ...
    async |_rpc: SimpleRequestRpc, block, _, _, change_view: ViewPair| {
      // Make sure the change can pick up its output
